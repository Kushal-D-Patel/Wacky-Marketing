...
User *create_user(char *name) {
    ...
    FriendNode *fn = malloc(sizeof(FriendNode));
    if (fn == NULL) {
        printf("Memory allocation failed\n");
        exit(EXIT_FAILURE);
    }
    ...
    FriendNode *cur;
    for (cur = allUsers; cur->next != NULL && strcmp(cur->next->user->name, name) < 0; cur = cur->next)
        ;
...
int delete_user(User *user) {
    FriendNode *cur, *prev = NULL;

    for (cur = allUsers; cur != NULL; prev = cur, cur = cur->next) {
        if (cur->user == user) {
            if (prev != NULL) {
                prev->next = cur->next;
            } else {
            ...
            BrandNode *tempBrand = user->brands;
            while (tempBrand != NULL) {
                BrandNode *nextBrand = tempBrand->next;
                free(tempBrand);
                tempBrand = nextBrand;
            }
...
int add_friend(User *user, User *friend) {
    if (user == friend) {
        printf("Cannot add oneself as a friend\n");
        return -1;
    }
...
int follow_brand(User *user, char *brand_name) {
    ...
    if (in_brand_list(user->brands, brand_name)) {
        printf("Already following this brand\n");
        return -1;
    }
...
int unfollow_brand(User *user, char *brand_name) {
    ...
    if (!in_brand_list(user->brands, brand_name)) {
        printf("Not following this brand\n");
        return -1;
    }
...
int get_mutual_friends(User *a, User *b) {
    int count = 0;
    for (FriendNode *fa = a->friends; fa != NULL; fa = fa->next) {
        for (FriendNode *fb = b->friends; fb != NULL; fb = fb->next) {
            if (fa->user == fb->user) {
                count++;
                break;
            }
...
int get_degrees_of_connection(User *a, User *b) {
    ...
    int degree = 0;
    FriendNode *queue = malloc(sizeof(FriendNode));
    if (queue == NULL) {
    ...
    while (queue != NULL) {
        ...
        while (queue != NULL) {
            User *curUser = queue->user;
            if (curUser == b) {
                free(queue);
                return degree;
            }
            for (FriendNode *f = curUser->friends; f != NULL; f = f->next) {
                if (!f->user->visited) {
                    f->user->visited = true;
...
User *get_suggested_friend(User *user) {
    ...
    for (FriendNode *cur = allUsers; cur != NULL; cur = cur->next) {
        ...
        if (potentialFriend == user || in_friend_list(user->friends, potentialFriend))
            continue;
        int mutualFriends = get_mutual_friends(user, potentialFriend);
        if (mutualFriends > maxMutualFriends || (mutualFriends == maxMutualFriends && strcmp(potentialFriend->name, suggestedFriend->name) > 0)) {
...
int follow_suggested_brands(User *user, int n) {
    int followedBrands = 0;
    for (int i = 0; i < n; i++) {
        int maxSimilarities = -1;
        ...
        for (int j = 0; j < MAT_SIZE; j++) {
            if (!in_brand_list(user->brands, brand_names[j])) {
                ...
                for (BrandNode *brand = user->brands; brand != NULL; brand = brand->next) {
                    int index = get_brand_index(brand->brand_name);
                    if (index != -1 && brand_adjacency_matrix[index][j] == 1)
                        similarities++;
                }
                if (similarities > maxSimilarities || (similarities == maxSimilarities && strcmp(brand_names[j], suggestedBrand) > 0)) {
                    suggestedBrand = brand_names[j];
                    maxSimilarities = similarities;
                }
            }
        }
        if (suggestedBrand != NULL && follow_brand(user, suggestedBrand) == 0)
            followedBrands++;
    }
